...
const fuction = require('./lib/fuction')
    ...
    try {
        ...
        case 'stiker':
            if (isMedia) {
                ...
                const url = args[0]
                if (!url.match(isUrl)) client.reply(from, 'maaf mint, link yang kamu kirim tidak valid.', id)
                await client.sendStickerfromUrl(from, url)
                    .then((r) => {
                        if (!r && r !== undefined) client.sendText(from, 'maaf mint, link yang kamu kirim tidak memuat gambar.')
                    })
            } else {
                client.reply(from, 'tidak ada gambar mint! Untuk membuka daftar perintah kirim #menu', id)
            }
            break
        case 'tiktok': {
            if (args.length !== 1) return client.reply(from, 'maaf mint, link yang kamu kirim tidak valid', id)
            const url = args[0]
            if (!url.match(isUrl) && !url.includes('tiktok.com')) return client.reply(from, 'maaf mint, link yang kamu kirim tidak valid', id)
            await client.sendText(from, '*Scraping Metadata...*')
            await tiktok(url)
                .then((videoMeta) => {
                    const filename = videoMeta.authorMeta.name + '.mp4'
                    const caps = `*Metadata:*\nUsername: ${videoMeta.authorMeta.name} \nMusic: ${videoMeta.musicMeta.musicName} \nView: ${videoMeta.playCount.toLocaleString()} \nLike: ${videoMeta.diggCount.toLocaleString()} \nComment: ${videoMeta.commentCount.toLocaleString()} \nShare: ${videoMeta.shareCount.toLocaleString()} \nCaption: ${videoMeta.text.trim() ? videoMeta.text : '-'} \n\nDonasi: kontak admin dengan cara ketik #donasi \nTerimakasih.`
                    ...
                    client.reply(from, 'gagal mengambil metadata mit, link yang kamu kirim tidak valid', id)
                })
        }
            break
        case 'ig':
        case 'instagram': {
            if (args.length !== 1) return client.reply(from, 'maaf mint, link yang kamu kirim tidak valid', id)
            const url = args[0]
            if (!url.match(isUrl) && !url.includes('instagram.com')) return client.reply(from, 'maaf mint, link yang kamu kirim tidak valid', id)
            await client.sendText(from, '*Scraping Metadata...*')
            instagram(url)
                .then(async (videoMeta) => {
                    ...
                    client.sendText(from, `Link Download mint:\n${content.join('\n')} \n\nDonasi: kontak admin dengan cara ketik #donasi \nTerimakasih.`)
                }).catch((err) => {
                    if (err == 'Not a video') return client.reply(from, 'error mint, tidak ada video di link yang kamu kirim', id)
                    client.reply(from, 'error mint, user private atau link salah', id)
                })
        }
            break
        case 'twt':
        case 'twitter': {
            if (args.length !== 1) return client.reply(from, 'maaf mint, link yang kamu kirim tidak valid', id)
            const url = args[0]
            if (!url.match(isUrl) & !url.includes('twitter.com') || url.includes('t.co')) return client.reply(from, 'maaf mint, url yang kamu kirim tidak valid', id)
            await client.sendText(from, '*Scraping Metadata...*')
            twitter(url)
                .then(async (videoMeta) => {
                    try {
        ...
        case 'facebook': {
            if (args.length !== 1) return client.reply(from, 'maaf mint, link yang kamu kirim tidak valid', id)
            const url = args[0]
            if (!url.match(isUrl) && !url.includes('facebook.com')) return client.reply(from, 'maaf mint, url yang kamu kirim tidak valid', id)
            await client.sendText(from, '*Scraping Metadata...*')
            facebook(url)
                .then(async (videoMeta) => {
                    try {
                ...
                .catch((err) => {
                    client.reply(from, `error mint, url tidak valid atau tidak memuat video \n\n${err}`, id)
                })
        }
            break
        case 'quoteit':
            const quoteIT = await fuction.quoteIT()
                if (quoteIT) {
                    await client.sendFileFromUrl(from, quoteIT)
            }
            break
        case 'mim':
        case 'memes':
